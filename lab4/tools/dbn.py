...
class DeepBeliefNet:
    ...
    def __init__(self, sizes, image_size, n_labels, batch_size):
        ...
        self.history = {"label_convergence": []}

        return

    def recognize(self, true_img, true_lbl, n_steps: int = 15):
        ...
        self.history["label_convergence"] = []
        h_prob, h_act = self.rbm_stack["vis--hid"].get_h_given_v_dir(vis)
        v_prob, v_act = self.rbm_stack["hid--pen"].get_h_given_v_dir(h_act)
        v_act = np.concatenate((v_act, lbl), axis=1)

        for _ in range(n_steps):
            h_prob, h_act = self.rbm_stack["pen+lbl--top"].get_h_given_v(v_act)
            v_prob, v_act = self.rbm_stack["pen+lbl--top"].get_v_given_h(h_act)
            self.history["label_convergence"].append(
                np.argmax(v_act[:, -true_lbl.shape[1] :], axis=1)
            )

        predicted_lbl = v_act[:, -true_lbl.shape[1] :]
    ...
    def generate(self, true_lbl, name):
        ...
        vis = np.random.choice([0, 1], self.sizes["vis"]).reshape(-1, self.sizes["vis"])

        hid_act = self.rbm_stack["vis--hid"].get_h_given_v_dir(vis)[1]
        pen_act = self.rbm_stack["hid--pen"].get_h_given_v_dir(hid_act)[1]
        pen_lbl_act = np.concatenate((pen_act, lbl), axis=1)

        for _ in range(self.n_gibbs_gener):
            top_act = self.rbm_stack["pen+lbl--top"].get_h_given_v(pen_lbl_act)[1]
            pen_lbl_act = self.rbm_stack["pen+lbl--top"].get_v_given_h(top_act)[1]
            pen_lbl_act[:, -lbl.shape[1] :] = lbl[:, :]
            pen_act_top_bot = pen_lbl_act[:, : -lbl.shape[1]]
            hid_act_top_bot = self.rbm_stack["hid--pen"].get_v_given_h_dir(
                pen_act_top_bot
            )[1]
            vis = self.rbm_stack["vis--hid"].get_v_given_h_dir(hid_act_top_bot)[1]
        ...
        anim = stitch_video(fig, records).save(
            "%s.generate%d.gif" % (name, np.argmax(true_lbl))
        )
    ...
    def train_greedylayerwise(self, vis_trainset, lbl_trainset, n_iterations):
        ...
        try:
            ...
            """
            self.rbm_stack["vis--hid"].untwine_weights()
            h_prob, h_act = self.rbm_stack["vis--hid"].get_h_given_v_dir(vis_trainset)
            self.rbm_stack["hid--pen"].cd1(h_act, n_iterations)
            ...
            """
            self.rbm_stack["hid--pen"].untwine_weights()
            h_prob, h_act = self.rbm_stack["hid--pen"].get_h_given_v_dir(h_act)
            h_act = np.concatenate((h_act, lbl_trainset), axis=1)
            self.rbm_stack["pen+lbl--top"].cd1(h_act, n_iterations)
